...
# the reason — and think first about whether the fixture needs the value.
SITE_PUBLIC_IDS = ()
...
CAPTURES_IN_HISTORY_DECIDED = {
    ...
    # sponsored slot. All of it is Google Finance's own rather than ours — no
...
def secret_check():
    ...
    # A scan that read nothing passes for the wrong reason, and that is not
    # hypothetical: this check asks GIT for its file list, so in a directory
    # that is not yet a repository — a fresh clone of a tarball, a
    # bootstrapped repo before `git init`, a CI checkout with a broken
    # fetch — `git ls-files` returns nothing and the scan reported
    # "ok, 0 files scanned, nothing credential-shaped". That is the exact
    # shape of failure CLAUDE.md §22 describes: a guard that quietly starts
    # passing once its input disappears.
    #
    # Deliberately keyed on ZERO rather than on a threshold. A repo can
    # legitimately be small; it cannot legitimately be empty.
    if scanned == 0:
        failed.append(
            "the credential scan read 0 files. It asks `git ls-files` for "
            "its list, so this usually means this directory is not a git "
            "repository (or the index is unreadable) — not that there is "
            "nothing to scan. Run `git init && git add -A` first.")
