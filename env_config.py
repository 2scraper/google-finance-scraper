...
ENV_KEYS = {
    # Shared across this family: one key, four separately-billed 2Captcha
    # products.
    "TWOCAPTCHA_KEY": "twocaptcha_key",
    # Per-site, and the prefix is load-bearing: a variable without it is
    # silently ignored by this loader, which is a setting that looks
    # configurable and is not.
    "GOOGLE_FINANCE_CDP_ENDPOINT": "cdp_endpoint",
    "GOOGLE_FINANCE_PROXY": "proxy",
    "GOOGLE_FINANCE_URL": "url",
    # Safe to map because `--market` has no default. A variable mapped onto a
    # flag that DOES have one is inert — the loader only fills unset values —
    # which is why `--out` is deliberately absent from this table.
    "GOOGLE_FINANCE_MARKET": "market",
}
...
def unknown_keys(path=None):
    ...
    Usually a typo — `TWO_CAPTCHA_KEY`, `GOOGLE_FINANCE_CDP` — which otherwise fails
